...
export default function ForgotPasswordModal({ isOpen, onClose, onPasswordResetSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white w-full max-w-md rounded-3xl p-6 md:p-8 shadow-2xl border border-[#ab3500]/20 space-y-6 relative">
        <button
          onClick={handleClose}
        ...
        <div className="text-center space-y-2">
          <div className="w-14 h-14 rounded-2xl bg-[#ff6b35]/15 text-[#ab3500] flex items-center justify-center mx-auto text-2xl font-bold">
            🔒
          </div>
          <h2 className="text-2xl font-extrabold text-[#261814]">
            {step === 1 && 'Forgot Password?'}
            {step === 2 && 'Enter Verification Code'}
            {step === 3 && 'Set New Password'}
            {step === 4 && 'Password Reset Complete!'}
          </h2>
          <p className="text-xs text-[#594139] leading-relaxed max-w-xs mx-auto">
            {step === 1 && 'Enter your registered Gmail address to receive a 6-digit password reset code.'}
        ...
        {step === 1 && (
          <form onSubmit={handleSendCode} className="space-y-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-[#261814]">Gmail Address</label>
              <input
                type="email"
                required
                value={email}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-[#ab3500] hover:bg-[#ff6b35] text-white rounded-full text-sm font-bold shadow-md transition-all cursor-pointer disabled:opacity-50"
            >
        ...
        {step === 2 && (
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-[#261814]">6-Digit Verification Code</label>
              <input
                type="text"
                required
                maxLength={6}
                ...
                placeholder="123456"
                className="w-full px-4 py-3 bg-[#fff1ed] rounded-xl outline-none text-center text-lg tracking-[0.5em] font-mono font-bold text-[#261814] border border-[#ab3500]/20 focus:ring-2 focus:ring-[#ab3500]/20"
              />
            </div>
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex-1 py-3 bg-[#ab3500] hover:bg-[#ff6b35] text-white rounded-full text-xs font-bold shadow-md transition-all cursor-pointer disabled:opacity-50"
              >
        ...
        {step === 3 && (
          <form onSubmit={handleResetPassword} className="space-y-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-[#261814]">New Password</label>
              <input
                type="password"
                required
                minLength={6}
